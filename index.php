...
<html>
<head>
  <title>Reddit giveaway decider</title>
  <link rel='stylesheet' href='style.css' />
</head>

<body>
  <h1>Reddit giveaway decider</h1>
  <form method="POST" action='index.php'>
	...
	<label>Enter text required in comment (leave blank to accept any text): <input name='textToFind' type='text' value='<?php echo strip_tags($_POST["textToFind"]); ?>' /></label><br />
	<label>Enter mininum account age in days (leave blank to accept any age): <input name='accountAge' type='number' value='<?php echo strip_tags($_POST["accountAge"]); ?>' /></label>
  ...
  <?php
    ...
    if (isset($_POST['getWinner'])){
      ...
      if ($comments != null && $_POST['upperBound'] > 0){
        ...
        for ($i=0;$i<count($comments[1]["data"]["children"]);$i++){ // parse comments and add to list of parsed comments
        ...
        if ($_POST['accountAge'] != ""){
          // check account age
          $userDetails = json_decode(json_encode($r->getUser($winnerUname)), true); 
          $accountAge = $userDetails["data"]["created_utc"];
          if ( time() - ($_POST['accountAge']*(60*60*24)) > $accountAge){
            // account is old enough
          } else{
            $accountOldEnough = false;
          }
        }
        ...
        if ($accountOldEnough){
          ...
          echo "<p>A winner was chosen (/u/$winnerUname), but their account age was below the minimum specified.</p><p>Please pick again (sorry about this - this is to minimise reddit API calls)</p>";
        }
		...
		if ($_POST['accountAge'] != ""){
		  // check account age
		  $userDetails = json_decode(json_encode($r->getUser($author)), true); 
		  $accountAge = $userDetails["data"]["created_utc"];
	    if ( time() - ($_POST['accountAge']*(60*60*24)) > $accountAge){
			  // account is old enough
		  } else{
		...
		if ($accountOldEnough){
		  ...
		  echo "<p>A winner was chosen (/u/$author), but their account age was below the minimum specified.</p><p>Please pick again (sorry about this - this is to minimise reddit API calls)</p>";
		}
	  }
    }
  ?>
  <p><br /></p>
  <hr />
	<p style="font-size: 12pt; color: black;">NOTICE: When giving away games with a number in the title (e.g. Fallout 4) ask for participants to use Roman numerals (e.g. Fallout IV) to avoid messing up the comment parser.</p>
